...
pub(crate) struct RepoCacheStage {
    live_dir: PathBuf,
...
pub(crate) fn prepare_staged_repo_cache(repo_cache_dir: &Path) -> anyhow::Result<RepoCacheStage> {
...
pub(crate) fn commit_staged_repo_cache(stage: RepoCacheStage) -> anyhow::Result<()> {
