...
impl SqliteTrustedIndex {
    ...
    pub fn open_sqlite(
        ...
        debug!(
            ...
            "inventory trusted index opened"
        );
        Ok(Self {
            store,
            db_path,
    ...
    pub fn protected_local_paths(&self) -> Vec<PathBuf> {
    ...
    #[instrument(level = "debug", skip(self))]
    pub fn get_segment_locations(&self, sig: &SegmentSignature) -> Result<Vec<SegmentLocation>> {
        let start = Instant::now();
    ...
    pub fn get_segment_locations_batch(
    ...
    ) -> Result<Vec<Vec<SegmentLocation>>> {
        if sigs.is_empty() {
            return Ok(Vec::new());
        }
        ...
        let start = Instant::now();
        let out = self.with_conn(|conn| {
            let mut out_by_sig: std::collections::HashMap<SegmentSignature, Vec<SegmentLocation>> =
                std::collections::HashMap::new();
            ...
            while let Some(r) = rows.next()? {
                ...
                out_by_sig.entry(key).or_default().push(SegmentLocation {
                    rel_path: PathBuf::from(rel),
                    offset: Self::i64_to_u64(start_i64),
                    len: Self::i64_to_u64(len_i64),
                });
...
#[cfg(test)]
mod tests {
    use super::{FinalizedFileRecord, SegmentLocation, SegmentSignature, SqliteTrustedIndex};
    use std::path::{Path, PathBuf};
    ...
    fn sorted_locs(mut locs: Vec<SegmentLocation>) -> Vec<(String, u64, u64)> {
        locs.sort_by(|a, b| {
            a.rel_path
                .cmp(&b.rel_path)
    ...
    fn open_inventory(
        ...
        SqliteTrustedIndex::open_sqlite(db_path, "inv", root_path).expect("open")
    }
    ...
    #[test]
    fn protected_local_paths_returns_db_relative_path_under_root() {
        let dir = tempdir().expect("tempdir");
