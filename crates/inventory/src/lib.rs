...
impl FleetInventory {
    /// Open the fresh, target-bound observation database and hold its session
    /// lock for the lifetime of this store.
    pub fn open(
        db_path: &Path,
        target_root: &Path,
        profile: ProfileId,
    ...
    fn connection(&self) -> Result<Connection> {
        let conn = open_connection(&self.db_path).map_err(inventory_error)?;
        Ok(conn)
    }
}
