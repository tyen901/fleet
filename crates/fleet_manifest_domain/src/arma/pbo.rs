...
pub fn partition_pbo<R: BufRead + Seek>(
    ...
    if meta.entries.first().is_some_and(|e| e.data_size != 0) {
        return Err(PboPartitionError::Invalid("first entry has non-zero size"));
    }
    ...
    for entry in meta.entries.iter().skip(1) {
        ...
        if offset.saturating_add(size) > file_len {
            return Err(PboPartitionError::Invalid("entry exceeds file length"));
        }
        if size > 0 {
    ...
    if offset < file_len {
        parts.push((offset, file_len - offset));
    }
