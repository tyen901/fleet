...
fn parse_legacy_srf(text: &str) -> Result<ModManifest> {
    ...
    let checksum = Md5Digest::parse_hex(checksum_hex).map_err(|e| anyhow::anyhow!("{e:?}"))?;
    ...
    for _ in 0..file_count {
        ...
        let checksum =
            Md5Digest::parse_hex(file_checksum_hex).map_err(|e| anyhow::anyhow!("{e:?}"))?;
        ...
        for _ in 0..part_count {
            ...
            let pchk = pp.next().context("legacy srf: missing part checksum")?;
            let pchk = Md5Digest::parse_hex(pchk).map_err(|e| anyhow::anyhow!("{e:?}"))?;
