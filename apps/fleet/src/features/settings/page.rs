...
use crate::ui::components::{ButtonVariant, ConfirmModal};
...
use super::sections::{appearance_section, arma_section, reset_section, support_section};
...
pub fn Settings() -> Element {
    ...
    let is_show_advanced_options_non_default = fleet_core::settings_field_is_non_default(
        fleet_core::SettingsField::ShowAdvancedOptions,
        &snap.settings,
        &defaults,
    );
    let default_inventory_ignore = defaults.sync.inventory_ignore_rules.clone();
    ...
    let mut inventory_ignore_draft =
        use_signal(|| snap.settings.sync.inventory_ignore_rules.clone());
    let mut inventory_ignore_last_synced =
        use_signal(|| snap.settings.sync.inventory_ignore_rules.clone());
    let inventory_ignore_save_seq = use_signal(|| 0_u64);
    if inventory_ignore_last_synced() != snap.settings.sync.inventory_ignore_rules {
        inventory_ignore_last_synced.set(snap.settings.sync.inventory_ignore_rules.clone());
        inventory_ignore_draft.set(snap.settings.sync.inventory_ignore_rules.clone());
    }
    ...
    let bridge_for_show_advanced_options = bridge.clone();
    ...
    let on_set_inventory_ignore = move |next: String| {
        ...
        spawn_debounced_settings_save(
            ...
            |settings, value| settings.sync.inventory_ignore_rules = value,
        );
    };
    ...
    let on_reset_inventory_ignore = move || {
        ...
        spawn_settings_task(toasts, "Reset sync ignore rules", async move {
            let mut settings = bridge.get_snapshot().settings.clone();
            settings.sync.inventory_ignore_rules = defaults;
            bridge.core().settings_save(settings).await
        });
    };
    ...
    let toasts_for_show_advanced = toasts.clone();
    let on_toggle_show_advanced_options = move |next: bool| {
        spawn_settings_update(
            bridge_for_show_advanced_options.clone(),
            toasts_for_show_advanced.clone(),
            move |settings| {
                settings.ui.show_advanced_options = next;
            },
        );
    };
        ...
        div { class: "page page--scroll settings-page",
            div { class: "page__inner stack-lg",
                ...
                {support_section(
                    ...
                    snap.settings.sync.inventory_ignore_rules.trim() != default_inventory_ignore.trim(),
                    ...
                    snap.settings.ui.show_advanced_options,
                    is_show_advanced_options_non_default,
                    on_toggle_show_advanced_options,
                    open_logs,
                    restart_onboarding,
                )}
